...
try {
  ...
  console.log(
    `   Enabled: ${debugLiteEnabled ? "🔴 YES (SLOW)" : "🟢 NO (FAST)"}`
  );

  if (debugLiteEnabled) {
    console.warn(
      "   ⚠️ RECOMMENDATION: Disable debugLite by setting debugLite._enabled = false"
    );
  } else {
...
try {
  if (window.appState && window.appState.channelState) {
    ...
    if (
      window.appState.channelState.analog &&
      window.appState.channelState.analog.lineColors
    ) {
      const oldColor = window.appState.channelState.analog.lineColors[0];
      window.appState.channelState.analog.lineColors[0] =
      ...
      if (elapsed > 50) {
        ...
        console.log(
          "   3. Full chart recreation (check chart._channelIndices)"
        );
      } else {
    ...
    console.log(
      "⚠️ Could not access app state. Make sure it's exposed globally."
    );
...
console.log("");
console.log(
  "Then watch the [Performance] logs in console when changing colors."
);
console.log("");
...
console.log("\n");
console.log(
  "%c✅ Diagnostic complete. Check results above.",
