...
export function analyzeGroupsAndPublishMaxYAxes(charts, channelState, cfg) {
  try {
    ...
    const groupsWithAxisCounts = Array.from(
      ...
      (_, groupId) => {
        // Find all channel indices in this group
        const groupIndices = userGroups
          .map((g, idx) => (g === groupId ? idx : -1))
          .filter((idx) => idx >= 0);

        // Get channel configs for this group
        const groupChannels = groupIndices.map(
          (idx) => cfg?.analogChannels?.[idx] || {}
        );

        // Calculate how many Y-axes this group needs
        const axisCount = calculateAxisCountForGroup(groupChannels);
        ...
        return {
          ...
          axisCount: axisCount,
          units: groupChannels.map((ch) => ch.unit || "?"),
        };
      }
    );
    ...
    const groupSummary = groupsWithAxisCounts
      .map(
        (g) =>
          `G${g.id}(${g.channelCount}ch,${g.axisCount}ax:[${g.units.join(
            ","
          )}])`
      )
      .join(" | ");
