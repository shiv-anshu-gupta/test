...
Complete flow of the COMTRADE Visualization Application from file loading through user interaction.
...
graph TB
    ...
    subgraph FILE_LOADING["File Loading and Parsing Layer"]
        ...
        PARSE_DAT --> DATA_OBJ[(Data Object<br/>time float array<br/>analogData 2D array<br/>digitalData 2D array<br/>startDateInfo)]
    end
...
### 🟦 **File Loading and Parsing Layer (Blue)**

- **parseCFG**: Extracts COMTRADE configuration (channels, sampling rates, time info)
- **parseDAT**: Parses data files with linear interpolation for uniform time spacing
- Creates foundational data objects for the application

### 🟪 **Reactive State Management (Purple)**

- **channelState**: Stores all channel metadata (colors, labels, groups, scales, units, etc.)
- **dataState**: Stores actual signal data (time arrays and analog/digital values)
- **verticalLinesX**: Tracks measurement marker positions across all charts
- Uses Proxy-based reactive system for automatic updates

### 🟩 **Chart Rendering Pipeline (Green)**

- **renderComtradeCharts**: Main orchestrator function
- **autoGroupChannels**: Intelligent grouping by name patterns, units, phases
- **createChartOptions**: Configures uPlot chart with axes, series, grid settings
- **initUPlotChart**: Creates uPlot instances with plugins
- **Add Plugin Suite**: Applies visualization plugins (zoom, pan, delta measurement, vertical lines)
- Renders both analog and digital charts separately

### 🟥 **Reactive Update Manager (Red)**

- **subscribeChartUpdates**: Sets up state change listeners
- **In-place updates**: Color, label, data inversion without full recreate
- **Full recreate**: When grouping or scale changes
- **Time window updates**: Handles start/duration changes with X-axis scaling
- **forceRedraw**: Triggers visual refresh after updates

### 🟧 **Channel Editor - Tabulator Integration (Orange)**

- **showChannelListWindow**: Opens popup editor window
- **Tabulator table**: Interactive table for editing channel properties
- **postMessage**: Cross-window communication between parent and child
- **History system**: Undo/Redo support with localStorage persistence
- **Add/Delete channels**: Dynamic channel management

### 🟦 **User Interaction Layer (Teal)**

- **Mouse events**: Tooltip display, vertical line toggling
- **Drag events**: Chart reordering via drag-and-drop
- **Keyboard shortcuts**: Undo (Ctrl+Z), Redo (Ctrl+Y)
- **Vertical line sync**: Coordinated updates across all charts

### 🟩 **Advanced Features and Plugins (Pink)**

- **autoUnitScalePlugin**: Dynamic SI prefix calculation
- **verticalLinePlugin**: Synchronized measurement markers
- **deltaBoxPlugin**: Time and value difference measurements
- **horizontalZoomPanPlugin**: Coordinated zoom/pan operations
- **digitalFillPlugin**: Rectangle rendering for digital signals
- **History tracking**: State mutation recording and localStorage persistence
- **DOM binding**: Two-way data synchronization
- **Computed properties**: Derived value calculation
- **Middleware**: State change interception and validation

### 🟩 **Utility Functions and Helpers (Light Green)**

- **makeAxisValueFormatter**: Format axis tick labels with SI prefixes
- **getSiPrefix**: Calculate appropriate SI prefix (k, M, G, T)
- **extractUnit**: Parse units from channel labels
- **getNearestIndex**: Binary search for closest time value
- **createTooltip**: Generate tooltip DOM elements
- **Chart utilities**: Container creation, cleanup, ResizeObserver
- **Digital utilities**: Filter static channels, optimize display

---

## Key Features Highlighted

### ✨ **Linear Interpolation (in parseDAT)**

- Calculates uniform time spacing using: `time = sampleNumber / samplingRate`
- Supports multiple sampling rates with smooth transitions
- Ignores DAT file timestamps for consistency

### 🔄 **Reactive System**

- State changes automatically trigger chart updates
- No manual refresh required
- Efficient in-place updates for performance

### 📊 **Multi-Format Support**

- ASCII and BINARY COMTRADE file formats
- Analog and digital channels
- Multiple sampling rates in single file

### 🎨 **Advanced Visualization**

- SI unit prefixes (μ, m, k, M, G, T)
- Synchronized vertical measurement lines
- Delta value calculations between markers
- Interactive zoom/pan/drag operations

### 💾 **Data Persistence**

- localStorage-based undo/redo history
- PDF export of channel configurations
...
1. ✅ **Embedded in JSDoc** comments for function documentation
2. ✅ **Included in README.md** for project overview
3. ✅ **Added to Mermaid documentation** in `/docs` folder
4. ✅ **Used in presentations** and technical discussions
5. ✅ **Referenced in GitHub** issues and pull requests
...
- See `docs/FLOWCHART_INDEX.md` for other flowcharts
- See `src/components/comtradeUtils.js` for parsing logic
- See `src/components/chartManager.js` for reactive system
- See `src/main.js` for main application logic
