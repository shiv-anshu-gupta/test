...
export function createChartOptions({
  ...
  return {
    ...
    width,
    height,
    scales: (Object.keys(scales).length > 0 ? scales: {
    ...
    axes: [
      {
        ...
        label: `${xLabel}(${xUnit})`,
        grid: { show: true },
        values: makeAxisValueFormatter(xUnit || extractUnit(xLabel) || "sec", xScaleVal),
      },
      ...(
        singleYAxis
          ? [
              {
                ...
                grid: { show: true },
                values: makeAxisValueFormatter(yUnits[0] || extractUnit(yLabels[0]), axesScales[1] || 1),
              }
            ]
          : yLabels.map((label, idx) => {
    ...
    plugins: [
      autoUnitScalePlugin({ axesScales: axesScales, autoScaleUnit: autoScaleUnit }),
    ...
    if (opts.scales.y) {
      ...
      Object.keys(opts.scales).forEach(key => {
        if (key.startsWith("y")) {
          ...
          console.log(`[${context}] opts.scales.${key}.auto:`, opts.scales[key].auto);
        }
      });
    }
  } else {
