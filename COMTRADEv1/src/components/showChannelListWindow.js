...
//   // --- Load Tabulator (light by default) ---
//   const tabulatorCSS = win.document.createElement("link");
//   tabulatorCSS.id = "tabulator-theme-link";
//   tabulatorCSS.rel = "stylesheet";
//   tabulatorCSS.href =
//     "https://unpkg.com/tabulator-tables@5.5.2/dist/css/tabulator.min.css";
//   win.document.head.appendChild(tabulatorCSS);
...
//   tabulatorScript.src =
//     "https://unpkg.com/tabulator-tables@5.5.2/dist/js/tabulator.min.js";
//   tabulatorScript.onload = () => setTimeout(() => setupChannelListOnce(), 100);
//   win.document.head.appendChild(tabulatorScript);

//   // --- Body structure ---
//   win.document.body.innerHTML = `
//   <div id="channel-table" class="w-auto flex flex-col gap-4 rounded-md h-auto p-2 md:p-4 transition-colors duration-300 bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-100">
//     <div id="button-bar" class="flex flex-wrap gap-2 m-2 md:m-3 items-center">
//       <select id="group-select" class="border rounded px-2 py-1 text-sm dark:bg-gray-800 dark:border-gray-600">
//         <option value="Analog">Analog</option>
//         <option value="Digital">Digital</option>
//       </select>
//       <button id="add-row" class="bg-green-500 hover:bg-green-600 text-white text-sm px-3 py-1 rounded">Add Blank Row</button>
//       <button id="history-undo" class="bg-green-500 hover:bg-green-600 text-white text-sm px-3 py-1 rounded">Undo</button>
//       <button id="history-redo" class="bg-green-500 hover:bg-green-600 text-white text-sm px-3 py-1 rounded">Redo</button>
//       <button id="download-pdf" class="bg-green-500 hover:bg-green-600 text-white text-sm px-3 py-1 rounded">Download PDF</button>
//       <button id="theme-toggle" class="ml-auto bg-gray-200 dark:bg-gray-700 dark:text-gray-100 text-gray-800 px-3 py-1 rounded text-sm transition">Toggle Theme</button>
//     </div>
//     <div id="channel-root" class="w-auto overflow-y-auto dark:border-gray-700 dark:bg-gray-800"></div>
//   </div>
//   `;

//   const style = win.document.createElement("style");
//   style.textContent = `body { font-family: sans-serif; }`;
//   win.document.head.appendChild(style);
...
//   // ---------------------------------------------
//   //  ⭐ Create table only once (NO recreate)
//   // ---------------------------------------------
//   function setupChannelListOnce() {
//     if (tableCreated) return; // IMPORTANT
//     tableCreated = true;
...
//   // -------------------------------------------------------
//   //  ⭐ Theme Toggle WITHOUT resetting Tabulator
//   // -------------------------------------------------------
//   win.document.addEventListener("click", (e) => {
//     if (e.target.id === "theme-toggle") {
//       const html = win.document.documentElement;
//       const linkEl = win.document.getElementById("tabulator-theme-link");

//       if (currentTheme === "light") {
//         html.classList.add("dark");
//         currentTheme = "dark";
//         linkEl.href =
//           "https://unpkg.com/tabulator-tables@5.5.2/dist/css/tabulator_midnight.min.css";
//       } else {
...
export function showChannelListWindow(
  ...
  // Add custom CSS to ensure MathLive keyboard has high z-index
  ...
  // --- Initialize MathLive after DOM is ready ---
  mlScript.onload = () => {
    setTimeout(() => {
      // Get the math-field element
      const mfEl = win.document.getElementById("mf");

      if (mfEl) {
        // Get the Mathfield object from MathLive
        const mathfield = win.MathfieldElement.forElement(mfEl);

        // Set default value
        mathfield.setValue("\\frac{a}{b}");

        // Listen for changes
        mathfield.on("input", () => {
          const latex = mathfield.getValue("latex");
          const plain = mathfield.getValue("plain-text"); // 🔥 WORKS 100%
