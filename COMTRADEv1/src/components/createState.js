...
export function createState(initialState, { batch = true } = {}) {
  ...
  proxy.subscribe = (fn, options) => {
    ...
    if (typeof options === 'string' || Array.isArray(options)) {
      ...
      if (options.path) pathArr = Array.isArray(options.path) ? options.path : options.path.split('.');
    ...
    if (pathArr) {
      wrapped = (change) => {
        if (
          ...
          (!descendants && change.path.length === pathArr.length && change.path.every((k, i) => k === pathArr[i]))
        ) {
          try { fn(change); } catch (e) { console.error('Subscriber error:', e); }
        }
      };
    } else if (selector) {
      ...
      wrapped = (change) => {
        ...
        if (newValue !== lastValue) {
          lastValue = newValue;
          try { fn({ ...change, selectorValue: newValue }); } catch (e) { console.error('Subscriber error:', e); }
        }
      };
    } else {
      wrapped = (change) => { try { fn(change); } catch (e) { console.error('Subscriber error:', e); } };
    }
